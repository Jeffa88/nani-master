...
class Solver:
    ...
    def _cross_left_or_right(self, edge_piece, face_piece, face_color, move_1, move_2):
        ...
        if edge_piece.pos.z == 0:
            ...
            # ne postavljajte potez poništavanja ako komad počinje u pravom položaju
            # (sa pogrešnom orijentacijom) ili ćemo zeznuti ostatak algoritma
            if edge_piece.pos.x != face_piece.pos.x:
                undo_move = "{0} {0}".format(cw)

        assert edge_piece.pos.z == -1

        # komad je na z = -1, rotirajte da ispravite lice (LIJEVO ili DESNO)
        count = 0
        ...
        # ako smo pomjerili ispravno postavljen komad (polje), vratite ga
        if undo_move:
            self.move(undo_move)

        # komad (polje) je na ispravnoj strani u ravni z = -1, ali ima dvije orijentacije
